...
list_profiles() {
    ...
    ls -1 *.wtprof | sed 's/\.wtprof//' | sed 's/^/\t/'
    exit 0
}
...
if [ "$PROFILE" = "--help" ] || [ "$PROFILE" = "-h" ] || [ -z "$PROFILE" ]; then
    ...
    list_profiles
elif [ ! -f "$PROJ_DIR/tests/$PROFILE.wtprof" ]; then
    echo "No such profile or option \"$PROFILE\" found"
    exit 1
else
    WTPROF="$PROJ_DIR/tests/$PROFILE.wtprof"
...
for part in $(cat $WTPROF); do
    ...
    if [[ $part =~ ^BOOT  ]]; then
        ...
        FS_TYPE=$(echo $part | awk 'BEGIN{ FS="\t" } { print $3 }')
        case $FS_TYPE in
            ext2 | ext3 | ext4)
